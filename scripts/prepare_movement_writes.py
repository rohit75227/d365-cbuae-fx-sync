#!/usr/bin/env python3
"""
Shard the per-period movement_<key>.json files (from build_monthly_movement.py)
into write_db-ready "period doc" + "shard doc" files for the "movement"
collection (Current Month Dr/Cr, consumed by the report's Company TB tab),
mirroring prepare_backfill_writes.py's approach for the "tb" collection.

Usage:
  python3 scripts/prepare_movement_writes.py --in-dir /tmp/movement_out --out-dir /tmp/movement_out/writes
"""
import argparse
import json
...
def main():
    ...
    for key in periods:
        ...
        all_writes.append({"op": "set", "collection": "movement", "doc_id": key, "file_path": str(period_file), "_bytes": period_file.stat().st_size})

        for i, shard in enumerate(shards):
            shard_file = out_dir / f"shard_{key}_{i}.json"
            with open(shard_file, "w") as f:
                json.dump({"rows": shard}, f)
            all_writes.append({"op": "set", "collection": f"movement/{key}/shards", "doc_id": str(i), "file_path": str(shard_file), "_bytes": shard_file.stat().st_size})
